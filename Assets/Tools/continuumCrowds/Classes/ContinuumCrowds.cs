		...
		for (int n = (int)goalPos.x; n < (int)(goalPos.x + goalDim.x); n++) {
			for (int m = (int)goalPos.y; m < (int)(goalPos.y + goalDim.y); m++) {
				...
				if ((n == (int)goalPos.x) ||
					...
					(m == (int)goalPos.y) ||
					(m == (int)(goalPos.y + goalDim.y) - 1)) { considered.Enqueue(loc, 0f); }
			}
		}
	...
	void calculatePotentialGradientAndNormalize()
	{
		for (int i = Nloc; i < (Ndim + Nloc); i++) {
			for (int k = Mloc; k < (Mdim + Mloc); k++) {
				if ((i != Nloc) && (i != (Ndim + Nloc) - 1) && (k != Mloc) && (k != (Mdim + Mloc) - 1)) { writeNormalizedPotentialGradientFieldData(i, k, i - 1, i + 1, k - 1, k + 1); } // generic spot
				...
				else if ((i == (Ndim + Nloc) - 1) && (k == (Mdim + Mloc) - 1)) { writeNormalizedPotentialGradientFieldData(i, k, i - 1, i, k - 1, k); }   // bottom right corner
